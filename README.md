...
如果 Implementation / Testing / Deployment 阶段的 gate 发现上游 authority 文档必须先修，使用 `codespec authority-repair begin ...` 进入修复态。修复态只允许修改 `meta.yaml`、`authority-repairs/*.yaml` 和声明的最小 authority 路径；`close` 会重跑记录的 gate 并通过 metadata consistency 后关闭 repair。完整框架 smoke 仍应在常规回归中执行，但不再作为每次 close 的内置步骤。未进入修复态时，`design.md`、`spec.md` 等越权修改仍会被阻断。
