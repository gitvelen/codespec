...
2. 读取当前目录 `./CLAUDE.md`
3. 读取当前目录 `./meta.yaml`
4. 检查当前 Git 分支是否与 `execution_branch` 一致；若 `execution_branch != null` 且不一致，必须停下并先同步上下文
...
9. 只有在默认层不足以解释当前任务时，才继续下钻 `./spec-appendices/` 或 `./design-appendices/`
...
## Gate / Hook 提醒
- Requirements：优先关注 `spec-completeness`
- Design：优先关注 `design-structure-complete`（别名：`design-readiness`）
- Implementation：优先关注 `implementation-ready`；`pre-commit` 仍会兜底 `branch-alignment`、`metadata-consistency`、`scope`、`boundary`
- Testing：优先关注 `trace-consistency`、`verification`
- Deployment：优先关注 `deployment-readiness`、`promotion`
